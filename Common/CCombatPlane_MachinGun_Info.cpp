...
CCombatPlane_MachinGun_Info::CCombatPlane_MachinGun_Info()
...
/////////////////////////////////////////////////////////////////////////////////////////
// Update()
void CCombatPlane_MachinGun_Info::Update(float _fDTime)
{
	...
	if (m_blive){
		...
		//	지형과의 충돌.
		//======================================================
		//float y = g_pTerrain->GetHeight(m_vPos);
		//if (m_vPos.y <= y){
		//	m_vPos.y = y;
		//	m_blive = false;
		//}
		...
		//else{
			...
			missile.fAxisLen[0] = 1.f;
			missile.fAxisLen[1] = 1.f;
			missile.fAxisLen[2] = 1.f;

			missile.type = OBJ_BULLET;		// 자신의 타입을 총알(공격) 오브젝트로 설정

			for (int j = MAX_SESSION - 1; j >= 0; j--) // 접속한 수 만큼
			{
	...
	// 데이터 전송.
	g_ServerIocp.GetConnectUserManager()->WriteGameClientAll(SCOMBATPLANE_MACHINGUN_TM, pData, 73);

}

void CCombatPlane_MachinGun_Info::ComUpdate(float _fDTime)
{
	Update(_fDTime);
}